...
from utils import load_data_new, load_snli_data
from networks import SimCSEModelSup
...
# 加载数据集
datasets_sts = {
    'train': load_data_new('./train.data'),
    'test': load_data_new('./test.data'),
    'valid': load_data_new('./test.data')
}
...
class TrainDataset(Dataset):
    ...
    def text_2_id(self, text: str):
        return tokenizer([text[0], text[1], str(text[2])], max_length=hp.MAXLEN,
...
if __name__ == '__main__':    
    ...
    train_data = datasets_sts['train']
    random.shuffle(train_data)     

    dev_data = datasets_sts['valid']
    test_data = datasets_sts['test']
    #
